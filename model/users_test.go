...
func init() {
	env, err := barque.NewEnvironment(context.Background(), &barque.Configuration{
...
func (s *UserTestSuite) SetupTest() {
	...
	for _, user := range s.users {
		s.Require().NoError(user.Save(ctx, s.env))
	}
}
...
func (s *UserTestSuite) TearDownTest() {
	ctx, cancel := s.env.Context()
	defer cancel()
	s.Require().NoError(s.env.DB().Collection(userCollection).Drop(ctx))
}
...
func (s *UserTestSuite) TestGetOrAddUser() {
	...
	fromDB, err := FindUser(ctx, s.env, "NewUser")
	s.Require().NoError(err)
...
func (s *UserTestSuite) TestPutLoginCache() {
	...
	u1, err := FindUser(ctx, s.env, s.users[0].ID)

	s.Require().NoError(err)
	s.Equal(s.users[0].ID, u1.ID)

	u2, err := FindUser(ctx, s.env, s.users[1].ID)
	s.Require().NoError(err)
	...
	s.NoError(err)
	u1Updated, err := FindUser(ctx, s.env, s.users[0].ID)
	s.Require().NoError(err)
	...
	s.NoError(err)
	u5, err := FindUser(ctx, s.env, s.users[2].ID)
	s.Require().NoError(err)
